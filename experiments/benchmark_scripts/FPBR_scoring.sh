...
python -u ${EXE_DIR}/exe/test.py \
  ...
  data.FPBR_scoring.root_data_dir=${DATA_DIR} \
  data.FPBR_scoring.key_dir=${EXE_DIR}/keys/FPBR/scoring \
  data.FPBR_scoring.test_result_path=${BENCHMARK_DIR}/benchmark/result_scoring_${EPOCH}.txt \
  run.num_workers=4
date
